"""Document service: listing, serving files, and HTML report generation."""
...
from app.core.graph_state import load_graph
from app.services.audit_service import get_log


def list_documents(project_id: int, db: Session) -> list[dict]:
...
def serve_document(filename: str, project_id: int, db: Session) -> Response | None:
    """Serve an uploaded document file. Returns None if not found."""
...
def generate_report(project_id: int, db: Session) -> str:
    """Generate an HTML forensic intelligence report."""
